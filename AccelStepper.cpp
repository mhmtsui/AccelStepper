...
boolean AccelStepper::runSpeedwithAcceleration()
{
    if (runSpeed())
    computeNewSpeedT();
    return _speed != 0.0;
...
void AccelStepper::computeNewSpeedT()
{
    long speedTo = _targetSpeed - _speed; // +ve is clockwise from curent location
    ...
    if (speedTo > 0)
    {
        ...
        {
            // Currently accelerating, need to decel now? Or maybe going the wrong way?
            if (_direction == DIRECTION_CCW)// Start deceleration
                _n = -stepsToStop;
        }
        else if (_n < 0)
        {
            // Currently decelerating, need to accel again?
	        if (_direction == DIRECTION_CW)// Start accceleration
                _n = -_n;
        }
    ...
    else if (speedTo < 0)
    {
        ...
        {
            // Currently accelerating, need to decel now? Or maybe going the wrong way?
            if (_direction == DIRECTION_CW)// Start deceleration
                _n = -stepsToStop;
        }
        else if (_n < 0)
        {
            // Currently decelerating, need to accel again?
	        if (_direction == DIRECTION_CCW)// Start accceleration
                _n = -_n;
        }
    ...
    if (_n == 0)
    {
        // First step from stopped
        _cn = _c0;
        _direction = (speedTo > 0) ? DIRECTION_CW : DIRECTION_CCW;
    }
    else
    {
        // Subsequent step. Works for accel (n is +_ve) and decel (n is -ve).
...
void AccelStepper::setTargetSpeed(float targetSpeed)
{
    if (targetSpeed == _targetSpeed)
        return;
    targetSpeed = constrain(targetSpeed, -_maxSpeed, _maxSpeed);
    _targetSpeed = targetSpeed;
    computeNewSpeedT();
}

float AccelStepper::targetSpeed()
{
    return _targetSpeed;
}
